...
/**
 * 스프링은 Text 타입과 Binary타입의 핸들러를 지원하는데, 우리는 채팅 서비스를 만들 것이므로
 * TextWebSocketHandler를 상속하여 WebSocketHandler를 만들어 줄 것이다.
 * 우리가 메세지를 json형식을 통해서 웹소켓을 통해 서버로 보내면, Handler는 이를받아 ObjectMapper를 통해서
